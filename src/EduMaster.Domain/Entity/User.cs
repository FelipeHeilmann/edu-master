...
{  
    ...
    protected User(Guid id, string name, Email email, Password password, Phone phone, CPF cpf, string role, string status, DateTime birthDate, DateTime enrollmentDate, DateTime createdAt)
    {
        Id = id;
        Name = name;
        Status = status;
    ...
    public static User Create(string name, string email, string password, string phone, string cpf, string role, DateTime birthDate, DateTime enrollmentDate)
    {
        ...
        return new User(Guid.NewGuid(), name, new Email(email), BcryptPassword.Create(password), new Phone(phone), new CPF(cpf), role, status, birthDate, enrollmentDate, DateTime.UtcNow);
    }
